...
- `tools/fuzz/` —— `cargo-fuzz` 目标，针对 lexer/parser 喂随机输入找 panic（呼应 OPTIVE_REVIEW 勘误后的 UB 收敛记录）。
- `tools/check-stdlib-coverage.py` —— 核对 `docs/stdlib.md` 列出的每个 API 是否在 `src/stdlib/` 注册。
